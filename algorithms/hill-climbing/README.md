...
```mermaid
flowchart TD
    Start([開始]) --> Init[初期解を設定<br/>x = 0.0]
    Init --> Evaluate[現在の解を評価<br/>current_value = f&#40;current_x&#41;]
    Evaluate --> Generate[近傍解を生成<br/>neighbor = current ± random&#40;stepSize&#41;]
    Generate --> EvalNeighbor[近傍解を評価<br/>neighbor_value = f&#40;neighbor&#41;]
    EvalNeighbor --> Compare{neighbor_value ><br/>current_value?}
    Compare -->|Yes| Update[解を更新<br/>current = neighbor]
    Compare -->|No| Check{反復終了?}
    Update --> Check
    Check -->|No| Generate
    Check -->|Yes| Output[最終解を出力]
    Output --> End([終了])
```
...
```mermaid
flowchart TD
    Start([開始]) --> GenInit[複数の初期解を生成]
    GenInit --> Loop{各初期解について}
    Loop -->|次の初期解| HC[Hill Climbing 実行]
    HC --> Record[最良解を更新<br/>&#40;必要な場合&#41;]
    Record --> Loop
    Loop -->|完了| Output[最良解を出力]
    Output --> End([終了])
```
